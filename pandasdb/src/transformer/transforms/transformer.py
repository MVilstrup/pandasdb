...
from pandasdb.src.transformer import TransformDAG
from pandasdb.src.transformer import AggregationContainer, ColumnContainer
from pandasdb.src.transformer import ColumnGenerationError, EmptyDataFrameError, EmptyColumnError
from pandasdb.src.transformer import TransformationCore, Union
import pandas as pd
from pandasdb.src.transformer import to_df
...
class Transformer(TransformationCore):
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            raise EmptyDataFrameError("PandasDB cannot transform and empty dataframe, please look at either your initialize funtion or the inputs to the transformer")

        """ Apply all pre conditions one at a time """
        df = self._apply_pre_conditions(df)
        if df.empty:
            raise EmptyDataFrameError("The pre_conditions lead to an empty dataframe which cannot be handled by PandasDB, please change the pre_conditions or the inputs to the transformer")
